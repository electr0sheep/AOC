...
def map_antinodes(antennae, y, x, antinodes)
  ...
  antennae.each_with_index do |row, y2|
    row.each_with_index do |column, x2|
      ...
      antinodes[y][x] << signal
      antinodes[y2][x2] << signal
      ...
      loop do
        multiplier += 1
        antinode1_index = y + slope[0] * multiplier, x + slope[1] * multiplier
        antinode2_index = y2 - slope[0] * multiplier, x2 - slope[1] * multiplier
        begin
          antinodes[antinode1_index[0]][antinode1_index[1]] << signal
        rescue IndexError
          begin
            antinodes[antinode2_index[0]][antinode2_index[1]]
          rescue IndexError
            break
          end
        end
